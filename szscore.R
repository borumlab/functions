...
szscore<-function(time_period_value, poplist){
...
baseline_sumsl<-aggregate(cbind(SEIZURE_LOAD_DAY)~MRNUMBER, data=baseline_szscore,sum)
...
baseline_days$time_period<-(baseline_sumsl$SEIZURE_LOAD_DAY/
                            baseline_days$freq
                            [match(baseline_sumsl$MRNUMBER, baseline_days$MRNUMBER)])*time_period
...
for(i in 1:length(patient_list)){ 
  ...
  list_emptydf_block <- lapply(chunk, function(block) sum(block$SEIZURE_LOAD_DAY, na.rm=TRUE)) 
  ...
  emptydf$SEIZURE_SCORE <- 100*emptydf$emptydf/baseline_days[,"time_period"][i] 
