...
mednumberscore<-function(time_period_value, poplist){
...
baseline_summn<-aggregate(cbind(MED_NUMBER_DAY)~MRNUMBER, data=baseline_medscore,sum)
...
baseline_days$time_period<-(baseline_summn$MED_NUMBER_DAY/
                            baseline_days$freq
                            [match(baseline_summn$MRNUMBER, baseline_days$MRNUMBER)])*time_period
...
for(i in 1:length(patient_list)){ 
  ...
  list_emptydf_block <- lapply(chunk, function(block) sum(block$MED_NUMBER_DAY, na.rm=TRUE)) 
  ...
  emptydf$MED_NUMBER_SCORE <- 100*emptydf$emptydf/baseline_days[,"time_period"][i] 
